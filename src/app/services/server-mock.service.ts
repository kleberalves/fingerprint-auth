...
export class ServerMockService {
  ...
  registerCredential(user: User, credential: Credential): boolean {
  ...
  extractAuthData(credential: Credential): Uint8Array {
   ...
   // const decodedAttestationObj: DecodedAttestionObj = CBOR.decode((credential.response as any).attestationObject);
    const decodedAttestationObj: DecodedAttestionObj = CBOR.decode(("TESTE" as any).attestationObject);
