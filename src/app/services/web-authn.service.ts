...
export class WebAuthnService {
  ...
  webAuthnSignup(user: User): Promise<Credential | null> {
    ...
    const publicKeyCredentialCreationOptions: PublicKeyCredentialCreationOptions = {
      ...
      user: {
        ...
        id: Uint8Array.from([user.id], (c) => { return c ? c.charCodeAt(0) : 0 }),
        name: user.email,
  ...
  webAuthnSignin(user: User): Promise<Credential | null> {
